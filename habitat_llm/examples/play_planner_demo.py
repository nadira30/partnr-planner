...
def run_planner(config, dataset: CollaborationDatasetV0 = None, conn=None):
    ...
    # cprint(f"{env_interface.full_world_graph.get_all_objects()}", "blue")
    # print(env_interface.full_world_graph.get_world_descr())
    env_interface.full_world_graph.get_all_furnitures()
    # print(f"\n[DEBUG] Found {len(furnitures)} furniture nodes in the world graph:")
    # for f in furnitures:
    #     try:
    #         print(
    #             f"  - Name: {f.name}, "
    ...
    #         print(f"  - Could not read attributes for {f}: {e}")
    # respectables = env_interface.full_world_graph.get_all_receptacles()
    # for r in respectables:
    #     try:
    #         print(f" - Name: {r.name}")
    #     except Exception as e:
    #         print(f"  - Could not read attributes for {r}: {e}")
    # print(env_interface.full_world_graph.get_all_receptacles())
    # print(env_interface.full_world_graph.get_all_rooms())
    ...
    # agent pose: eval_runner.env_interface.env.habitat_env.sim.AgentState
    ...
    if config.mode == "cli":
        # instruction = """
        #     - Overview:
        #         You are controlling Agent_0 in a house. Your goal is to complete a short **morning routine** by visiting different rooms and interacting with the environment. You must use valid actions in the correct order, ensuring smooth transitions between activities.

        #     - Instructions:
        #         1. Start from the bedroom, then move to the bathroom, and finally to the kitchen.
        #         2. You can use the following actions: `Navigate[room_name]`, `Explore[room_name]`, `Open[furniture_name]`, and `Wait[]`.
        #         3. Follow the logical order of a morning routine — wake up, freshen up, check the kitchen.
        #         4. Perform **one action at a time** and use only the provided rooms and furniture names.
        #         5. Stop after completing the final step and output `"Assigned!"`.

        #     - Example Morning Routine:
        #         Thought: Agent_0 starts the day in the bedroom, looks around, then goes to the bathroom to freshen up, moves to the kitchen to open a cabinet, explores for items, and waits to simulate having breakfast.

        #         Agent_0_Action_Sequence:
        #         - Navigate[bedroom_1]
        #         - Explore[bedroom_1]
        #         - Navigate[bathroom_1]
        #         - Explore[bathroom_1]
        ...
        #     - Stopword: "Assigned!"
        #     - End expression: "Final Thought:"

        # """
        instruction = (
            """
                         navigate agent_1 [bedroom_2], pick [toy_airplane_1] and place[toy_airplane_1,couch_18] and you are done. """
